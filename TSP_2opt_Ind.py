...
class Individual:
    def __init__(self, path: np.array=None, alpha: float=0.3):
        if path is None:
    ...
    def change_alpha(self):
        ...
        self.alpha = np.random.normal(self.alpha, 0.03)
...
def initialization_mix() -> np.ndarray:
    ...
    for i in range(lambdaa):
        ...
        if not tourIsCorrect(new_individual):
            print("NOT CORRECT new_individual: ", new_individual)
...
def selection_kTour(population, k):
    ...
    for i in range(mu):
        ...
        if not tourIsCorrect(selected[i].path):
            print("selected: ", selected[i].path)
...
def elimination_localSearch(joinedPopulation, countSameBestSol, i, tuning):
    ...
    for r in random_survivors:
        r.change_alpha

    # Concatenate the best and random survivors
    survivors = np.concatenate((best_selected, random_survivors))
    return survivors
